...
class Room {
    ...
    startSignalingSocket(username) {
     var connectionPromise = new Promise( (resolve, reject) => {
        this.ws[username] = new WebSocket('wss://localhost:8080')
...
class App extends React.Component {
  ...
  constructor(props) {
	...
	//This is called when Room.joinRoom() has initiated
	...
	//This is called when local mediastream is obtained , to get the local video do this.room.localStream[username]
	...
	//parameter contains User object
	
	//This is called when a new user joins, use the list this.room.connectedUsers get all connected users 
	...
	//Gives a list of users who are currently speaking
    };
  ...
  render() {
	//TO JOIN this.room.joinRoom(this.state.username, this.state.room, Room.VIDEO_CALL) for video call
	// FOR SCREENSHARE this.room.joinRoom(this.state.username, this.state.room, Room.SCREEN_SHARE)
	// To stop SCREENSHARE this.room.stopScreenShare(this.state.username)
	
	// To mute your mic or camera this.room.toggleUserAudio(this.state.username) , this.room.toggleUserVideo(this.state.username)
